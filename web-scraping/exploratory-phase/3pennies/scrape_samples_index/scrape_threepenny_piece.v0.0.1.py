...
def looks_like_bio_paragraph(text: str, author_name: str | None) -> bool:
    ...
    if author_name:
        ...
        if low.startswith(author_low + " "):
            return True
        if low.startswith(author_low + "'s "):
            return True
        if low.startswith(author_low + "’s "):
            return True
...
def main():
    parser = argparse.ArgumentParser(
        description="Scrape a single Threepenny Review piece page."
    )
    parser.add_argument("piece_url", help="Piece URL, often from /samples/")
    parser.add_argument("--out", default="threepenny_piece.json", help="Output JSON path")
