...
HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; EvergreenReviewScraper/0.2.3)",
...
def find_bio_for_name(soup, name, fallback_index=None):
    ...
    for h4 in h4s:
        ...
        low = txt.lower()
        if low == lowered:
            exact_matches.append(h4)
        elif lowered in low:
            partial_matches.append(h4)
    if exact_matches:
        target = exact_matches[-1]
    elif partial_matches:
        target = partial_matches[-1]
    elif fallback_index is not None and len(h4s) > fallback_index:
...
def extract_content(author_h3, art_p, soup, visible_author=None, visible_artist=None):
    start = art_p or author_h3
    if not start:
        return "", ""

    author_key = (visible_author or "").strip().lower()
    if not author_key:
        return "", ""

    matches = []
    for h4 in soup.find_all("h4"):
        label = norm_space(h4.get_text(" ", strip=True)) or ""
        low = label.lower()
        if low == author_key or author_key in low:
            matches.append(h4)
    stop_h4 = matches[-1] if matches else None

    blocks_html = []
    blocks_text = []
    seen = set()
    allowed = {"p", "blockquote", "ul", "ol", "pre", "figcaption"}

    for el in start.find_all_next(True):
        if el == art_p:
            continue
        if stop_h4 is not None and el == stop_h4:
            break
        if el.name == "h4":
            label = norm_space(el.get_text(" ", strip=True)) or ""
            low = label.lower()
            if stop_h4 is not None and el == stop_h4:
                break
            if low == author_key or author_key in low:
                continue
            if re.fullmatch(r"[IVXLC]+", label) or label in {"***", "*", "#"}:
        ...
        flat = norm_space(txt)
        if not txt or not flat:
            continue
        if re.match(r"^Art by\s+", flat, flags=re.I):
            continue
        if is_issue_text(flat):
            break

        key = re.sub(r"\s+", " ", flat).strip().lower()
        if key in seen:
            continue
        seen.add(key)
...
def split_issue_authors(author_value):
    if not author_value:
        return []
    text = norm_space(author_value) or ""
    if not text:
        return []
    parts = [norm_space(p) for p in text.split(",")]
    return [p for p in parts if p]


def add_manual_review_flag(notes):
    notes = list(notes or [])
    if any(n == "top_author_mismatch_issue_json_vs_page" or str(n).startswith("manual_review") for n in notes):
        if "manual_review" not in notes:
...
def scrape_piece(url, issue_label=None, issue_url=None, issue_number=None, input_title=None, input_author=None, input_type=None, order_in_section=None):
    ...
    try:
        ...
        return {
            ...
            "derived": {"author_bio_raw": None, "author_bios_raw": [], "visual_artist_bio_raw": None},
            "scrape_meta": {"scraped_at_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"), "scraper_version": "0.2.3", "notes": [f"request_failed: {e}"]},
        }
    ...
    author_h3, art_p, extracted_author, visible_artist = extract_visible_credits(soup)
    visible_author = input_author or extracted_author
    if input_author and extracted_author and norm_space(input_author) != norm_space(extracted_author):
        notes.append("top_author_mismatch_issue_json_vs_page")
    if input_author and not extracted_author:
    ...
    input_author_parts = split_issue_authors(input_author)
    if len(input_author_parts) > 1:
        author_bios_raw = [find_bio_for_name(soup, name, fallback_index=None) for name in input_author_parts]
    else:
        author_bios_raw = [author_bio] if author_bio else []
    ...
    content_html, content_text = extract_content(author_h3, art_p, soup, visible_author=visible_author, visible_artist=visible_artist)
    if not content_text:
    ...
    notes = add_manual_review_flag(notes)
