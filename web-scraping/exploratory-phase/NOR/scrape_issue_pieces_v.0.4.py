...
DEFAULT_HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; NORExploratoryScraper/0.4; +https://example.com)",
...
def extract_header(soup: BeautifulSoup) -> Dict[str, Any]:
    ...
    meta_node = soup.select_one("p.entry-meta") or soup.select_one(".entry-meta")
    ...
    author_nodes = []
    selectors = [
        'p.entry-meta a[href*="/writer/"]',
        '.entry-meta a[href*="/writer/"]',
        'a[rel="author"]',
    ]
    ...
    categories = []
    if meta_node:
        for a in meta_node.select(".entry-categories a[href], a[href*='/category/']"):
