...
def fetch(url):
    """Fetch a URL via curl, forcing UTF-8 decoding on all platforms."""
    r = subprocess.run(
        ["curl", "-sL", url, "-A", UA,
    ...
    # Decode bytes explicitly as UTF-8, replacing any undecodable chars
    # This avoids the cp1252 crash on Windows
...
def parse_piece(url, html, issue_context=None):
    ...
    # Extract content in two pieces:
    #   A) Title/byline block   — div.article-twenty-twenty-style-title-meta
    #   B) Prose body block     — div.article-twenty-twenty-style-content
    # End boundary: div.article-contributor--bottom (or footer / comments)

    # Locate the contributor bottom section as a hard end boundary
    end_boundary = len(html)
    for end_marker in [
        ...
        if idx > 0:
            # Walk back to the opening < of the enclosing div
            div_start = html.rfind('<div', 0, idx)
            if div_start > 0:
                end_boundary = div_start
            break

    # Slice the HTML to just the article region
    article_region = html[:end_boundary]

    title_meta_m = re.search(
        r'<div[^>]*class=["\']article-twenty-twenty-style-title-meta["\']>(.*?)</div>',
        article_region, re.DOTALL
    )

    # The prose content div — grab everything after the opening tag up to end_boundary
    ...
    if prose_start_m:
        prose_raw = article_region[prose_start_m.end():]
        # Close it at the first top-level </div> that brings depth back to 0
        depth = 1
        pos = 0
        while pos < len(prose_raw) and depth > 0:
            open_m  = re.search(r'<div[^>]*>', prose_raw[pos:])
            close_m = re.search(r'</div>', prose_raw[pos:])
            if open_m and (not close_m or open_m.start() < close_m.start()):
                ...
                break
        prose_inner = prose_raw[:pos]

        title_block = clean_html(title_meta_m.group(1)) if title_meta_m else ""
        ...
        # Fallback: collect all <p> tags from the article tag
